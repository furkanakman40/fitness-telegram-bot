...
async def beslenme(user=Depends(core.auth)):
    ...
    js='''<script>async function toggleMeal(key){const r=await fetch('/api/nutrition/toggle',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({key:key})});const j=await r.json();setTimeout(()=>location.reload(),300)}</script>'''
...
def program_for_offset(offset):
    ...
    if weekday<5:
        return core.PROGRAM[weekday]
    return ("Dinlenme","Aktif Toparlanma",[("Yürüyüş","30–45 dk"),("Hafif mobilite / esneme","10 dk")])


def workout_text(offset):
    day,title,exercises=program_for_offset(offset)
    when="BUGÜN" if offset==0 else "YARIN"
    lines=[f"🏋️ {when} — {day} / {title}",""]
    for i,(name,sets) in enumerate(exercises,1):lines.append(f"{i}. {name} — {sets}")
    lines += ["","🎯 Formu bozma. Setleri kontrollü yap; ağırlık ikinci planda."]
    return "\n".join(lines)
...
async def coach_message(update,context):
    ...
    if any(x in text for x in ["yarinin program","yarin program","yarin ne var","yarin ne yapacagim","yarin antrenman"]):
        await update.message.reply_text(workout_text(1)); return
    if any(x in text for x in ["bugunun program","bugun program","bugun ne var","bugun ne yapacagim","bugun antrenman","programi at"]):
        await update.message.reply_text(workout_text(0)); return
    if any(x in text for x in ["spor yapasim yok","antrenman yapasim yok","motivasyonum yok","usendim"]):
        await update.message.reply_text("Bugün hedefin mükemmel antrenman değil. Salona git, ilk 10 dakikayı yap. 10 dakika sonunda hâlâ kötüysen hafifletirsin; ama zinciri kırma. 💪"); return
    if "ne yesem" in text or "ne yiyim" in text or "yemek oner" in text:
        await update.message.reply_text("🥗 Basit seçim: yağsız protein + ölçülü karbonhidrat + sebze. Örn: 200 g tavuk, bulgur/pirinç ve büyük salata. Günlük toplamını /kalori ve /protein ile kaydet."); return
    if "hedef" in text:
        uid=core.get_or_create_user(update.effective_user); g=core.get_goals(uid)
        await update.message.reply_text(f"🎯 Hedeflerin\n⚖️ {g.get('target_weight_kg','—')} kg\n🔥 {g.get('daily_calories','—')} kcal\n🥩 {g.get('daily_protein_g','—')} g protein\n💧 {g.get('daily_water_l','—')} L su\n🚶 {g.get('daily_steps','—')} adım"); return
    await update.message.reply_text("Koç buradayım. Bana örneğin ‘bugünün programını at’, ‘yarının programını at’, ‘ne yesem’, ‘hedeflerim ne’ veya ‘spor yapasım yok’ yazabilirsin. 💪")
